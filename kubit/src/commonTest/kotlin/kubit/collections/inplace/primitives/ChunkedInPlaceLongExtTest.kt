...
class ChunkedInPlaceLongTest {
    ...
    fun `empty list returns empty chunked list`() {
        val list = emptyLongList().chunkedInPlace(1)
    ...
    fun `list smaller than chunk size returns one chunk`() {
        ...
        assertEquals(listOf(1L, 2L, 3L), subListToStdList(list[0]))
    }
    ...
    fun `list divisible by chunk size returns correct chunks`() {
        ...
        for (i in 0 until 10) {
            val expected = MutableList(12) { (i * 12 + it + 1).toLong() }
    ...
    fun `list not divisible by chunk size returns correct chunks`() {
        ...
        for (i in 0 until 6) {
            val expected = MutableList(19) { (i * 19 + it + 1).toLong() }
            assertEquals(expected, subListToStdList(list[i]))
        }

        val expected = MutableList(120 % 19) { (6 * 19 + it + 1).toLong() }
    ...
    fun `chunk size of 1 returns list of lists with single elements`() {
        ...
        for (i in 0 until 65) {
            assertEquals(listOf((i + 1).toLong()), subListToStdList(list[i]))
        }
    }
    ...
    fun `fastForEach iterates over all sublists`() {
        ...
        val collectedChunks = mutableListOf<List<Long>>()
    ...
    fun `fastForEachIndexed iterates with correct indices`() {
        ...
        val list = sourceList.chunkedInPlace(2)
        val expectedChunks = mutableListOf<List<Long>>()
    ...
    fun `forEachChunk iterates on the correct chunks`() {
        ...
        sourceList.forEachChunk(9) { chunk ->
            ...
            val expected = MutableList(size) { (start + it).toLong() }
    ...
    fun `forEachChunk list divisible by chunk size`() {
        val list = seqList(4)
        val chunks = mutableListOf<List<Long>>()
        list.forEachChunk(2) { chunks.add(subListToStdList(it)) }
        assertEquals(2, chunks.size)
        assertEquals(listOf(1L, 2L), chunks[0])
        assertEquals(listOf(3L, 4L), chunks[1])
    }
    ...
    fun `forEachChunk list not divisible by chunk size`() {
        ...
        assertEquals(listOf(1L, 2L), chunks[0])
        assertEquals(listOf(3L, 4L), chunks[1])
        assertEquals(listOf(5L), chunks[2])
    }
    ...
    fun `forEachChunk chunk size of 1`() {
        ...
        assertEquals(listOf(1L), chunks[0])
        assertEquals(listOf(2L), chunks[1])
        assertEquals(listOf(3L), chunks[2])
        assertEquals(listOf(4L), chunks[3])
        assertEquals(listOf(5L), chunks[4])
    }
    ...
    fun `forEachChunk chunk size larger than list size`() {
        ...
        assertEquals(listOf(1L, 2L, 3L), chunks[0])
    }
    ...
    fun `forEachChunkIndexed chunk size of 1`() {
        ...
        list.forEachChunkIndexed(1) { i, chunk ->
            assertEquals(listOf((i + 1).toLong()), subListToStdList(chunk))
        }
    }
