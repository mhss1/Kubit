...
class ChunkedInPlaceDoubleTest {
    ...
    fun `empty list returns empty chunked list`() {
        val list = emptyDoubleList().chunkedInPlace(1)
    ...
    fun `list smaller than chunk size returns one chunk`() {
        ...
        assertEquals(listOf(1.0, 2.0, 3.0), subListToStdList(list[0]))
    }
    ...
    fun `list divisible by chunk size returns correct chunks`() {
        ...
        for (i in 0 until 10) {
            val expected = MutableList(12) { (i * 12 + it + 1).toDouble() }
    ...
    fun `list not divisible by chunk size returns correct chunks`() {
        ...
        for (i in 0 until 6) {
            val expected = MutableList(19) { (i * 19 + it + 1).toDouble() }
            assertEquals(expected, subListToStdList(list[i]))
        }

        val expected = MutableList(120 % 19) { (6 * 19 + it + 1).toDouble() }
    ...
    fun `chunk size of 1 returns list of lists with single elements`() {
        ...
        for (i in 0 until 65) {
            assertEquals(listOf((i + 1).toDouble()), subListToStdList(list[i]))
        }
    }
    ...
    fun `fastForEach iterates over all sublists`() {
        ...
        val collectedChunks = mutableListOf<List<Double>>()
    ...
    fun `fastForEachIndexed iterates with correct indices`() {
        ...
        val list = sourceList.chunkedInPlace(2)
        val expectedChunks = mutableListOf<List<Double>>()
    ...
    fun `forEachChunk iterates on the correct chunks`() {
        ...
        sourceList.forEachChunk(9) { chunk ->
            ...
            val expected = MutableList(size) { (start + it).toDouble() }
    ...
    fun `forEachChunk list divisible by chunk size`() {
        val list = seqList(4)
        val chunks = mutableListOf<List<Double>>()
        list.forEachChunk(2) { chunks.add(subListToStdList(it)) }
        assertEquals(2, chunks.size)
        assertEquals(listOf(1.0, 2.0), chunks[0])
        assertEquals(listOf(3.0, 4.0), chunks[1])
    }
    ...
    fun `forEachChunk list not divisible by chunk size`() {
        ...
        assertEquals(listOf(1.0, 2.0), chunks[0])
        assertEquals(listOf(3.0, 4.0), chunks[1])
        assertEquals(listOf(5.0), chunks[2])
    }
    ...
    fun `forEachChunk chunk size of 1`() {
        ...
        assertEquals(listOf(1.0), chunks[0])
        assertEquals(listOf(2.0), chunks[1])
        assertEquals(listOf(3.0), chunks[2])
        assertEquals(listOf(4.0), chunks[3])
        assertEquals(listOf(5.0), chunks[4])
    }
    ...
    fun `forEachChunk chunk size larger than list size`() {
        ...
        assertEquals(listOf(1.0, 2.0, 3.0), chunks[0])
    }
    ...
    fun `forEachChunkIndexed chunk size of 1`() {
        ...
        list.forEachChunkIndexed(1) { i, chunk ->
            assertEquals(listOf((i + 1).toDouble()), subListToStdList(chunk))
        }
    }
