...
class ChunkedInPlaceFloatTest {
    ...
    fun `empty list returns empty chunked list`() {
        val list = emptyFloatList().chunkedInPlace(1)
    ...
    fun `list smaller than chunk size returns one chunk`() {
        ...
        assertEquals(listOf(1f, 2f, 3f), subListToStdList(list[0]))
    }
    ...
    fun `list divisible by chunk size returns correct chunks`() {
        ...
        for (i in 0 until 10) {
            val expected = MutableList(12) { (i * 12 + it + 1).toFloat() }
    ...
    fun `list not divisible by chunk size returns correct chunks`() {
        ...
        for (i in 0 until 6) {
            val expected = MutableList(19) { (i * 19 + it + 1).toFloat() }
            assertEquals(expected, subListToStdList(list[i]))
        }

        val expected = MutableList(120 % 19) { (6 * 19 + it + 1).toFloat() }
    ...
    fun `chunk size of 1 returns list of lists with single elements`() {
        ...
        for (i in 0 until 65) {
            assertEquals(listOf((i + 1).toFloat()), subListToStdList(list[i]))
        }
    }
    ...
    fun `fastForEach iterates over all sublists`() {
        ...
        val collectedChunks = mutableListOf<List<Float>>()
    ...
    fun `fastForEachIndexed iterates with correct indices`() {
        ...
        val list = sourceList.chunkedInPlace(2)
        val expectedChunks = mutableListOf<List<Float>>()
    ...
    fun `forEachChunk iterates on the correct chunks`() {
        ...
        sourceList.forEachChunk(9) { chunk ->
            ...
            val expected = MutableList(size) { (start + it).toFloat() }
    ...
    fun `forEachChunk list divisible by chunk size`() {
        ...
        assertEquals(listOf(1f, 2f), chunks[0])
        assertEquals(listOf(3f, 4f), chunks[1])
    }
    ...
    fun `forEachChunk list not divisible by chunk size`() {
        ...
        assertEquals(listOf(1f, 2f), chunks[0])
        assertEquals(listOf(3f, 4f), chunks[1])
        assertEquals(listOf(5f), chunks[2])
    }
    ...
    fun `forEachChunk chunk size of 1`() {
        ...
        assertEquals(listOf(1f), chunks[0])
        assertEquals(listOf(2f), chunks[1])
        assertEquals(listOf(3f), chunks[2])
        assertEquals(listOf(4f), chunks[3])
        assertEquals(listOf(5f), chunks[4])
    }
    ...
    fun `forEachChunk chunk size larger than list size`() {
        ...
        assertEquals(listOf(1f, 2f, 3f), chunks[0])
    }
    ...
    fun `forEachChunkIndexed chunk size of 1`() {
        ...
        list.forEachChunkIndexed(1) { i, chunk ->
            assertEquals(listOf((i + 1).toFloat()), subListToStdList(chunk))
        }
    }
