...
class LongDeque(initialCapacity: Int) {
    ...
    @PublishedApi
    @JvmField
    internal var data = LongArray(initialCapacity)
    ...
    fun firstOrNull(): Long? = if (_size == 0) null else data[head]
    ...
    fun lastOrNull(): Long? = if (_size == 0) null else data[lastIndex.internalIndex()]
    ...
    fun add(value: Long) = addLast(value)
    ...
    fun removeFirstOrNull(): Long? = if (_size == 0) null else removeFirst()
    ...
    fun removeLastOrNull(): Long? = if (_size == 0) null else removeLast()
...
fun LongDeque.asMutableList(): MutableList<Long> = LongDequeMutableListView(this)
/** Returns a live [List] view backed by this deque. */
fun LongDeque.asList(): List<Long> = asMutableList()

/** A [MutableList] view over a [LongDeque]. Mutating the view mutates the deque and vice versa. */
internal class LongDequeMutableListView(private val deque: LongDeque) : AbstractMutableList<Long>(), RandomAccess {
    override val size: Int
        get() = deque.size

    override fun get(index: Int) = deque[index]

    override fun set(index: Int, element: Long) = deque.set(index, element)

    override fun add(index: Int, element: Long) = deque.add(index, element)

    override fun removeAt(index: Int): Long = deque.removeAt(index)
}
