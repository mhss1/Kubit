...
class FloatDeque(initialCapacity: Int) {
    ...
    @PublishedApi
    @JvmField
    internal var data = FloatArray(initialCapacity)
    ...
    fun firstOrNull(): Float? = if (_size == 0) null else data[head]
    ...
    fun lastOrNull(): Float? = if (_size == 0) null else data[lastIndex.internalIndex()]
    ...
    fun add(value: Float) = addLast(value)
    ...
    fun removeFirstOrNull(): Float? = if (_size == 0) null else removeFirst()
    ...
    fun removeLastOrNull(): Float? = if (_size == 0) null else removeLast()
...
fun FloatDeque.asMutableList(): MutableList<Float> = FloatDequeMutableListView(this)
/** Returns a live [List] view backed by this deque. */
fun FloatDeque.asList(): List<Float> = asMutableList()

/** A [MutableList] view over a [FloatDeque]. Mutating the view mutates the deque and vice versa. */
internal class FloatDequeMutableListView(private val deque: FloatDeque) : AbstractMutableList<Float>(), RandomAccess {
    override val size: Int
        get() = deque.size

    override fun get(index: Int) = deque[index]

    override fun set(index: Int, element: Float) = deque.set(index, element)

    override fun add(index: Int, element: Float) = deque.add(index, element)

    override fun removeAt(index: Int): Float = deque.removeAt(index)
}
