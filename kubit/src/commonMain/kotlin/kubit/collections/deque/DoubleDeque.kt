...
class DoubleDeque(initialCapacity: Int) {
    ...
    @PublishedApi
    @JvmField
    internal var data = DoubleArray(initialCapacity)
    ...
    fun firstOrNull(): Double? = if (_size == 0) null else data[head]
    ...
    fun lastOrNull(): Double? = if (_size == 0) null else data[lastIndex.internalIndex()]
    ...
    fun add(value: Double) = addLast(value)
    ...
    fun removeFirstOrNull(): Double? = if (_size == 0) null else removeFirst()
    ...
    fun removeLastOrNull(): Double? = if (_size == 0) null else removeLast()
...
fun DoubleDeque.asMutableList(): MutableList<Double> = DoubleDequeMutableListView(this)
/** Returns a live [List] view backed by this deque. */
fun DoubleDeque.asList(): List<Double> = asMutableList()

/** A [MutableList] view over a [DoubleDeque]. Mutating the view mutates the deque and vice versa. */
internal class DoubleDequeMutableListView(private val deque: DoubleDeque) : AbstractMutableList<Double>(), RandomAccess {
    override val size: Int
        get() = deque.size

    override fun get(index: Int) = deque[index]

    override fun set(index: Int, element: Double) = deque.set(index, element)

    override fun add(index: Int, element: Double) = deque.add(index, element)

    override fun removeAt(index: Int): Double = deque.removeAt(index)
}
