...
/*
 * This file is based on the LongList from androidx.collection, but it has been
...
import kubit.collections.list.internal.EmptyLongArray
...
public sealed class LongList(initialCapacity: Int) {
     ...
     */
    public inline fun none(predicate: (element: Long) -> Boolean): Boolean {
     ...
     * iterating in the reverse order.
     */
    public inline fun reversedAll(predicate: (element: Long) -> Boolean): Boolean {
        contract { callsInPlace(predicate) }
     ...
     * @throws IllegalArgumentException if [n] is negative.
     */
    public fun take(n: Int): LongList {
        if (n < 0) throwIllegalArgumentException("n must be >= 0")
        val newSize = n.coerceAtMost(_size)
        return MutableLongList(newSize).also {
     ...
     * @return A [LongList] containing the initial contiguous elements matching [predicate].
     */
    public inline fun takeWhile(predicate: (Long) -> Boolean): LongList {
        contract { callsInPlace(predicate) }
     ...
     * @throws IllegalArgumentException if [n] is negative.
     */
    public fun takeLast(n: Int): LongList {
        if (n < 0) throwIllegalArgumentException("n must be >= 0")
        val newSize = n.coerceAtMost(_size)
        return MutableLongList(newSize).also {
     ...
     *
     * @param predicate Function that tests each element for a condition.
     * @return A [LongList] containing the final contiguous elements matching [predicate].
     */
    public inline fun takeLastWhile(predicate: (Long) -> Boolean): LongList {
        contract { callsInPlace(predicate) }
     ...
     * @throws IllegalArgumentException if [n] is negative.
     */
    public fun drop(n: Int): LongList {
        if (n < 0) throwIllegalArgumentException("n must be >= 0")
     ...
     *
     * @param predicate Function that tests each element for a condition.
     * @return A [LongList] containing the trailing elements after the initial contiguous elements matching [predicate].
     */
    public inline fun dropWhile(predicate: (Long) -> Boolean): LongList {
        contract { callsInPlace(predicate) }
        forEachIndexed { index, element ->
     ...
     * @throws IllegalArgumentException if [n] is negative.
     */
    public fun dropLast(n: Int): LongList {
        if (n < 0) throwIllegalArgumentException("n must be >= 0")
        val newSize = (_size - n).coerceAtLeast(0)
        return MutableLongList(newSize).also {
            content.copyInto(it.content, startIndex = 0, endIndex = newSize)
     ...
     *
     * @param predicate Function that tests each element for a condition.
     * @return A [LongList] with the trailing contiguous elements matching [predicate] removed.
     */
    public inline fun dropLastWhile(predicate: (Long) -> Boolean): LongList {
        contract { callsInPlace(predicate) }
        forEachReversedIndexed { index, element ->
     ...
     */
    public inline fun filter(predicate: (Long) -> Boolean): LongList {
        contract { callsInPlace(predicate) }
     ...
     */
    public fun copy(): LongList {
        return MutableLongList(_size).also {
     ...
     */
    public fun sortedDescending(): LongList {
     ...
     */
    public fun reversed(): LongList {
        return MutableLongList(_size).also { list ->
            forEachIndexed { index, element ->
     ...
     */
    public fun chunked(chunkSize: Int): List<LongList> {
        ...
        val result = ArrayList<LongList>(numChunks)

        var startIndex = 0
        while (startIndex < _size) {
            val endIndex = minOf(startIndex + chunkSize, _size)
            val chunk = MutableLongList(endIndex - startIndex).also { list ->
                content.copyInto(
     ...
     */
    public inline fun sumOf(element: (Long) -> Long): Long {
        var sum = 0L
     ...
     */
    public fun toLongArray(): LongArray = content.copyOf(_size)
    
    /**
     * Return a [MutableSet] of all elements.
     * The returned set does NOT guarantee the element iteration order of the list.
     */
    public fun toMutableSet(): MutableSet<Long> {
        return HashSet<Long>().also { set ->
            forEach { set.add(it) }
        }
    }
     ...
     * The returned set does NOT guarantee the element iteration order of the list.
     */
    public fun toSet(): Set<Long> = toMutableSet()
}
...
public class MutableLongList(
     ...
     * [MutableLongList] was changed or `false` if [elements] was empty.
     */
    public inline fun addAll(elements: List<Long>): Boolean {
        if (elements.isEmpty()) return false
        ensureCapacity(_size + elements.size)
        var curr = _size
     ...
     */
    private inline fun removeAll(predicate: (Long) -> Boolean): Boolean {
...
/**
 * Creates a [MutableLongList] with the specified size, where each element is calculated by calling the specified [init] function.
...
public inline fun MutableLongList(size: Int, init: (index: Int) -> Long): MutableLongList {
    return MutableLongList(size).apply {
...
/**
 * Creates a [LongList] with the specified size, where each element is calculated by calling the specified [init] function.
 * [init] is called for each list element sequentially starting from the first one. It should return the value for a list element given its index.
 *
 * @param size The initial capacity of the list.
 * @param init Function that takes an index and returns the element to be placed at that index.
...
/**
 * Builds a new [LongList] by populating a [MutableLongList] using the given
 * [builderAction].
 *
 ...
 */
public fun LongList?.orEmpty(): LongList = this ?: emptyLongList()
...
object LongListSerializer : KSerializer<LongList> {
    ...
    override val descriptor =
        buildSerialDescriptor("kubit.collections.list.LongList", StructureKind.LIST)

    override fun serialize(encoder: Encoder, value: LongList) {
        val size = value.size
    ...
    override fun deserialize(decoder: Decoder): LongList {
        ...
        val items: MutableLongList
        if (composite.decodeSequentially()) {
            items = MutableLongList(size)
            for (i in 0 until size) {
                items.content[i] = composite.decodeLongElement(descriptor, i)
            }
            items._size = size
        } else {
            items = MutableLongList(if (size == -1) 16 else size)
            var index = composite.decodeElementIndex(descriptor)
            while (index != CompositeDecoder.DECODE_DONE) {
                items.add(composite.decodeLongElement(descriptor, index))
