...
import kubit.collections.list.DoubleList
import kotlin.jvm.JvmField

/**
 * Represents a list of windows as a list of [InPlaceDoubleSubList] created from a source DoubleList.
 * Each window is a view into the original list. Changes in the original list will be reflected in the windows.
 *
 * @param sourceList The original DoubleList.
 ...
 */
class WindowedDoubleList(
    private val sourceList: DoubleList,
    private val windowSize: Int,
    private val stepSize: Int = 1,
    private val partialWindows: Boolean = false,
) : AbstractList<InPlaceDoubleSubList>() {
    ...
    /**
     * Internal array storing the windows as [InPlaceDoubleSubList] instances.
     */
    @PublishedApi
    @JvmField
     ...
     */
    inline fun fastForEach(action: (InPlaceDoubleSubList) -> Unit) {
     ...
     */
    inline fun fastForEachIndexed(action: (index: Int, InPlaceDoubleSubList) -> Unit) {
     ...
     */
    override fun get(index: Int): InPlaceDoubleSubList {
        checkIndex(index)
