...
import kubit.collections.list.LongList
import kotlin.jvm.JvmField

/**
 * Represents a list of windows as a list of [InPlaceLongSubList] created from a source LongList.
 * Each window is a view into the original list. Changes in the original list will be reflected in the windows.
 *
 * @param sourceList The original LongList.
 ...
 */
class WindowedLongList(
    private val sourceList: LongList,
    private val windowSize: Int,
    private val stepSize: Int = 1,
    private val partialWindows: Boolean = false,
) : AbstractList<InPlaceLongSubList>() {
    ...
    /**
     * Internal array storing the windows as [InPlaceLongSubList] instances.
     */
    @PublishedApi
    @JvmField
     ...
     */
    inline fun fastForEach(action: (InPlaceLongSubList) -> Unit) {
     ...
     */
    inline fun fastForEachIndexed(action: (index: Int, InPlaceLongSubList) -> Unit) {
     ...
     */
    override fun get(index: Int): InPlaceLongSubList {
        checkIndex(index)
